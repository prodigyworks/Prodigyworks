...
class SiteConfigClass {
	...
	public $emailfooter;
	public $adminrole;
	public $officeadminrole;
	public $officerole;
	public $officemanagerrole;
	public $compliancerole;
	public $areacoordinatorrole;
	public $managementrole;
	public $teamleaderrole;
	public $technicianrole;
	public $officepersonnelrole;
	public $trainingmanagementrole;
	public $technicianposition;
	public $teamleaderposition;
	public $currentoncallcodeid;
	public $systemmanagerid;
	public $regionalservicemanagerrole;
}
		...
		if (! isset($_SESSION['SITE_CONFIG'])) {
			...
			if ($result) {
				if (mysql_num_rows($result) == 1) {
					...
					$data->officeadminrole = $member['officeadminrole'];
					$data->officerole = $member['officerole'];
					$data->officemanagerrole = $member['officemanagerrole'];
					$data->compliancerole = $member['compliancerole'];
					$data->areacoordinatorrole = $member['areacoordinatorrole'];
					$data->regionalservicemanagerrole = $member['regionalservicemanagerrole'];
					$data->managementrole = $member['managementrole'];
					$data->teamleaderrole = $member['teamleaderrole'];
					$data->technicianrole = $member['technicianrole'];
					$data->technicianposition = $member['technicianposition'];
					$data->teamleaderposition = $member['teamleaderposition'];
					$data->currentoncallcodeid = $member['currentoncallcodeid'];
					$data->systemmanagerid = $member['systemmanagerid'];
					$data->officepersonnelrole = $member['officepersonnelrole'];
					$data->trainingmanagementrole = $member['trainingmanagementrole'];
...
//	$mail->Password = "Jasmin717440";           
	...
	if(!$mail->Send()) {
		...
		logError("Message sent OK", false);
		$error = 'Message sent!';
		return true;
	}
}
...
function sendRoleMessage($role, $subject, $message, $attachments = array()) {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'cpd@pestokill.co.uk', 'Pestokill Administration', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(), $attachments);
			...
			$qry = "INSERT INTO {$_SESSION['DB_PREFIX']}messages " .
					...
					"VALUES " .
					"(" . getSiteConfigData()->systemmanagerid . ", ". $member['member_id'] . ", '$subject', '$message', NOW(), 'N') ";
...
function sendInternalRoleMessage($role, $subject, $message, $attachments = array()) {
	$from = "cpd@pestokill.co.uk";
	$fromName = "Pestokill Administration";
...
function sendTeamMessage($id, $subject, $message, $footer = "") {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'cpd@pestokill.co.uk', 'Pestokill Administration', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(). $footer);
			...
			$qry = "INSERT INTO {$_SESSION['DB_PREFIX']}messages " .
					"(from_member_id, to_member_id, subject, message, createddate, status) " .
					"VALUES " .
					"(" . getSiteConfigData()->systemmanagerid . ", ". $id . ", '$subject', '$message', NOW(), 'N') ";
...
function sendUserMessage($id, $subject, $message, $footer = "", $attachments = array()) {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'cpd@pestokill.co.uk', 'Pestokill Administration', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(). $footer, $attachments);
