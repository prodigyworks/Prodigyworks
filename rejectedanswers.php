...
<div class='questionanswerss'>
	    	...
	    	<?php
				...
				if ($result) {
					while (($member = mysql_fetch_assoc($result))) {
						...
						echo "<td><a href='viewquestionanswer.php?id=" . $member['id'] . "'><img title='View question answers' src='images/view.png' /></a></td>";
					}
				} else {
					die($qry . " = " . mysql_error());
				}
	...
	<?php
		if ($prevpage) {
			echo "<img src='images/previouspage.png' onclick='window.location.href = \"rejectedquestionanswers.php?from=" . ($fromrow - $pagesize) . "&to=" . ($torow - $pagesize) . "\"' />";
		}
		
		if ($nextpage) {
			echo "<img src='images/nextpage.png' onclick='window.location.href = \"rejectedquestionanswers.php?from=" . ($fromrow + $pagesize) . "&to=" . ($torow + $pagesize) . "\"' />";
		}
	?>
</div>
<?php
