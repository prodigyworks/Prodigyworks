	...
	class DespatchReport extends FPDF
	{
		...
		function __construct($orientation, $metric, $size, $id) {
			...
			$sql = "SELECT A.*, DATE_FORMAT(A.signeddate, '%d/%m/%Y') AS signeddate,  " .
					...
					"E.street, E.town, E.city, E.county, E.postcode, AC.name AS stockname " .
			...
			if ($result) {
				while (($member = mysql_fetch_assoc($result))) {
					...
					$this->addHeading(90, 126, "REASON FOR LOAN");
					$this->addHeading(90, 162, "TERMS OF LOAN");
					$this->addLine(45, 170, "FROM  space for the date – or to write From date of receipt of the product");
					$this->addLine(45, 176, "UNTIL  Space for specific date – which as discussed will generate an automatic ");
					$this->addLine(45, 180, "reminder or if the loan relates to a repair – Until your own unit is back from the ");
					$this->addLine(45, 184, "factory");
					$this->addLine(45, 190, "BY ACCEPTING AND SIGNING THIS DOCUMENT YOU AGREE TO BE ");
					$this->addLine(45, 194, "RESPONSIBLE FOR THE ITEM(S) THAT WILL BE SENT TO YOU ON LOAN");
